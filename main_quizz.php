...
/* for the timer */
  ...
  var name = "timer"+"=";
    ...
    for(var dr=0; dr<ca.length; dr++) {
        var cr = ca[dr];
        while (cr.charAt(0)==' ') cr = cr.substring(1);

        if (cr.indexOf(name) == 0) 
          {
            cr=cr.substring(name.length, cr.length);
            break;
          }
    }
  var date=new Date();
  ...
  var total_time=Math.floor((cr-date)/1000);
  var temp_min=Math.floor(total_time/60);
  var temp_sec=Math.floor(total_time%60);
  min=temp_min;
  sec=temp_sec;
  }
  ...
  else
  {
    ...
    setTimeout(function(){ calMinute() }, 60000);
  ...
  function calSecond()
  {
    ...
    setTimeout(function(){ calSecond() }, 1000);
  }
